<script lang="ts">
  ...
  type Phase = 'loading' | 'exercises' | 'reviewing' | 'done' | 'error';
  ...
  onMount(() => {
    let init = false;
    const unsub = lang.subscribe(l => { if (init) loadAll(l); });
    loadAll(get(lang));
    init = true;
    return unsub;
  });

  async function loadAll(language = get(lang)) {
    phase = 'loading';
    try {
      const [exRes, shRes] = await Promise.all([
        apiFetch<{ exercises: Exercise[] }>(`/v1/output/exercises?language=${language}`),
        apiFetch<{ items: ShadowItem[] }>(`/v1/output/shadowing?language=${language}`),
      ]);
  ...
  async function submitAnswer() {
    ...
    try {
      ...
      toasts.add(err instanceof Error ? err.message : 'Failed to submit answer');
    }
    submitting = false;
  }
  ...
  function nextExercise() {
    ...
    if (currentIdx + 1 < exercises.length) { currentIdx++; phase = 'exercises'; }
    else { phase = 'done'; }
  }
  ...
  async function completeShadow(id: string) {
    try {
      ...
      toasts.add(err instanceof Error ? err.message : 'Failed to mark done');
    }
  }
...
<main>
  ...
  {#if activeTab === 'writing'}
    ...
    {:else if currentEx}
      ...
      <div class="exercise-card">
        ...
        {#if phase === 'exercises'}
          <textarea class="answer-input" placeholder="Type your answer here…" bind:value={answer} rows="4"></textarea>
          <div class="actions">
  ...
  {:else}
    {#if shadowItems.length === 0}
      <div class="msg"><p>No shadowing items yet. Mine cards with audio to build your queue.</p></div>
    {:else}
      <ul class="shadow-list">
        {#each shadowItems as item (item.id)}
          <li class="shadow-card">
            {#if item.card_front}<div class="shadow-word">{item.card_front}</div>{/if}
            {#if item.transcript}<p class="shadow-transcript">{item.transcript}</p>{/if}
...
<style>
  ...
  .answer-display { background: #13151a; border-left: 3px solid #4caf50; padding: 0.75rem 1rem; border-radius: 0 6px 6px 0; margin-bottom: 1rem; font-size: 0.95rem; white-space: pre-wrap; }
